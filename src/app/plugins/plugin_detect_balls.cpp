...
ProcessResult PluginDetectBalls::process ( FrameData * data, RenderOptions * options ) {
  ...
  // std::cout<<"COLORLIST "<<colorlist->getNumColorRegions()<<std::endl;
  if ( colorlist==0 ) {
    printf ( "error in ball detection plugin: no region-lists were found!\n" );
    return ProcessingFailed;
  }

  // std::cout<<"COLOR ID BALL "<<color_id_ball<<endl;
  // std::cout<<"NO OF REGIONS OF 2 color "<<colorlist->getRegionList(color_id_ball).getNumRegions();
  reg = colorlist->getRegionList ( color_id_ball ).getInitialElement();
  if(reg!=NULL){
    std::cout<<"REGION FOUND"<<std::endl;
    std::cout<<"REG WIDTH "<<reg->width();
    std::cout<<" HEIGHT "<<reg->height()<<endl;
  }
  ...
  if ( max_balls > 0 ) {
    ...
    // cout<<"NO. OF BALLS"<<result.size()<<endl;
